...
controllers.controller('LogController',['$scope', 'LogService', 'WebSocketService',
    function($scope, LogService, WebSocketService){
        ...
        $scope.startBots = function() { WebSocketService.command('bot', 'start', 'all') };
        $scope.stopBots = function() { WebSocketService.command('bot', 'stop', 'all') };
        ...
        $scope.newBot = function(values) { WebSocketService.command('bot', 'new', values) }

        $scope.commandResult = "";

        WebSocketService.subscribe('ruby', function(data){
...
app.factory('WebSocketService', ['$websocket', '$rootScope',
    function($websocket, $rootScope){
        ...
        return {
            ...
            command: function(controller, action, data){
                websocket.$emit(controller, {command: action, data: data})
            },
